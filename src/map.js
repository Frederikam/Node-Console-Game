...
rooms["1:0"] = new RoomLever(1, 0, "You see a leaver here lodged into the mountainside.");
...
module.exports.onLeverPull = function(){
	...
	if(leversPulled === 1){
		...
		rooms["-1:-1"] = new RoomLever(-1, -1, rooms["-1:-1"].getDescription() + "\nA second leaver has been revealed from the ground!");
	} else if(leversPulled === 2){
		console.log("Once again you hear a faint rumbling sound as you pull the second lever!");
		rooms["1:1"] = new RoomLever(1, 1, rooms["1:1"].getDescription() + "\nYou find that a third leaver has been revealed within the woods here!");
	} else if(leversPulled === 3){
		console.log("You pull the leaver and hear some very noisy grinding from the mountain!");
		rooms["0:-1"] = new GenericRoom(0, -1, rooms["0:-1"].getDescription() + "\nYou see that a opening in the rock wall has appeared.");
